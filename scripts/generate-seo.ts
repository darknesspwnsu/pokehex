...
const INDEX_PATH = path.join(REPO_ROOT, 'public', 'data', 'pokemon-index.json')
...
const FALLBACK_SITE_URL = `https://${FALLBACK_OWNER}.github.io/pokehex/`
...
const inferDefaultSiteUrl = () => {
  if (process.env.GITHUB_REPOSITORY_OWNER) {
    return `https://${process.env.GITHUB_REPOSITORY_OWNER}.github.io/pokehex/`
  }

  if (process.env.GITHUB_REPOSITORY) {
    const owner = process.env.GITHUB_REPOSITORY.split('/')[0]
    if (owner) {
      return `https://${owner}.github.io/pokehex/`
    }
  }
...
const buildUrl = (baseUrl: string, slug?: string) => {
  ...
  if (slug) {
    url.searchParams.set('pokemon', slug)
  }
  return url.toString()
}
